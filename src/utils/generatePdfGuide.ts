...
import { translations, PDFTranslation } from './pdfTranslations';
 ...
 * @param languageCode - Two-letter language code (default: 'en')
 */
export async function generatePdfGuide(languageCode: string = 'en'): Promise<void> {
    ...
    <html>
      ...
      <body>
        <div style="text-align: center; margin-bottom: 20px;">
          ...
          <p class="subtitle" style="margin: 8px 0 0 0;">${t.subtitle}</p>
        </div>
  ...
  try {
    ...
    const filename = languageCode === 'en' 
      ? 'PanHandler-Guide.pdf'
      : `PanHandler-Guide-${languageCode.toUpperCase()}.pdf`;
    ...
    if (await Sharing.isAvailableAsync()) {
      await Sharing.shareAsync(uri, {
        ...
        dialogTitle: `${t.title} (${languageCode.toUpperCase()})`,
      });
    } else {
